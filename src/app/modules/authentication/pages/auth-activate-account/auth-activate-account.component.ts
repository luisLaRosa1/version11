...
export class AuthActivateAccountComponent implements OnInit, AfterViewInit {
  ...
  changeNewPassword(): void {
    if (
      this.frmActivateAccount.controls['newPassword'].value.trim() == '' &&
      this.frmActivateAccount.controls['newPasswordConfirm'].value.trim() == ''
    ) {
  ...
  checkField(field: string, error: string): boolean {
    return (
      this.frmActivateAccount.controls[field].hasError(error) &&
      (this.frmActivateAccount.controls[field].touched ||
        this.frmActivateAccount.controls[field].dirty)
    );
  }

  validatorPasswordsEquals(): ValidationErrors | null {
