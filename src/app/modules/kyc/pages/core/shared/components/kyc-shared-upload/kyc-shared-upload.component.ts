...
import { IArchivo, IArchivoFolio } from 'src/app/modules/administracion/helpers/interfaces/core-carga-documental';
...
@Component({
  selector: 'app-kyc-shared-upload',
  templateUrl: './kyc-shared-upload.component.html',
  styleUrls: ['./kyc-shared-upload.component.css'],
})
export class KycSharedUploadComponent implements OnInit, OnChanges {
  @Input() documentos: any;
  @Input() titular: string = '';
  @Input() aseguradora: string = '';
  @Input() folio: IArchivoFolio = {} as IArchivoFolio;
  @Input() cotejo: boolean = false;
  ...
  displayedColumns: string[] = ['nombreOriginal', 'documento', 'operaciones'];
  ...
  setFormData(file: IArchivo): FormData {
    ...
    formData.append('cotejo', this.cotejo ? 'true': 'false');
    if(file.folio?.id){
      formData.append('folio', JSON.stringify(file.folio));
    }
    return formData;
  }
