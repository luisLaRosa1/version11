...
import { CountryType } from '../../administracion/helpers/interfaces/core-validacion-documental';
import {
  IKycArchivoValidacion,
  IKycValidacionDigital,
} from '../pages/core/helpers/interfaces/kyc-core-validacion-documental';

@Injectable({
  providedIn: 'root',
})
export class KycValidacionService {
  private readonly apiUrl = `${environment.urlApi}/core/validacion-digital`;
  private readonly apiUrlApoderdo = `${environment.urlApi}/catalogo/apoderado`;
  ...
  listAllCountryApoderado() {
    return this.http.get<IResponse<CountryType[]>>(
      `${environment.urlApi}/catalogos/pais/select-coincidence`
    );
  }
