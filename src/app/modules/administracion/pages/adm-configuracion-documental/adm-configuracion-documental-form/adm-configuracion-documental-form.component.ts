...
export class AdmConfiguracionDocumentalFormComponent implements OnInit {
  ...
  selectAseguradora(){
    this.frmConfiguracionDocumental.get('aseguradora')?.valueChanges.pipe(
      debounceTime(600),
      switchMap(value => this.ConfiguracionDocumentalService.getCatalogs(value || null)))
      .subscribe({
      ...
      this.ConfiguracionDocumentalService.getByIdAndGetCatalogosToEdit(
        ...
        next: (response: IResponse<IConfiguracionDocumentalEdit>) => {
          if (response.success) {
            ...
            const getNameAseguradora = this.aseguradoraAux.find((p) => p._id == configuracionDocumental.aseguradora);
            configuracionDocumental.aseguradora = getNameAseguradora?.nombreComercial??'';
  ...
  submit(): void {
    ...
    const arrayDocumentos = [EDocumento.FIC];
    const documentosFic = configuracionDocumental.documento.filter(
      (element: any) => arrayDocumentos.includes(element.clave)
    );

    if (documentosFic.length < 1) {
      const notificacion =
        'Es necesario seleccionar los documentos (Formato de identificación del cliente (FIC))';
    ...
    const ficValid = documentosFic.filter(
      (documento) => documento.clave === 'FIC'
    );
    ...
    const asegu = this.aseguradora.filter((element) => element.nombreComercial === configuracionDocumental.aseguradora);
    configuracionDocumental.aseguradora = asegu[0]._id;
