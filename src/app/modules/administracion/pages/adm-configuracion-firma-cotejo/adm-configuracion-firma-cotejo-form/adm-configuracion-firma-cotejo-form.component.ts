...
export class AdmConfiguracionFirmaCotejoFormComponent {
      ...
      next: (response) => {
        if (response.success) {
          ...
          if (
            ...
            this.load_data();
        } else this.#notifierService.error(response.message);
      },
      error: (err) => {
  ...
  change_proyecto(clave: string) {
    // this.#configLayout.getCountryForProject(clave).subscribe({
    //   next: (response) => {
    //     if (response.success) {
    //       this.proyecto = response.data;
    //       if (response.data.proceso == EProceso.FIANZAS) {
    //       } else {
    //         this.frmConfiguracion.controls['proyecto'].setValue(null);
    //         this.#notifierService.warning(
    //           'La configuración de firma cotejo solo esta disponible para proyectos de proceso <br><strong> Fianzas</strong>'
    //         );
    //       }
    //     }
    //   },
