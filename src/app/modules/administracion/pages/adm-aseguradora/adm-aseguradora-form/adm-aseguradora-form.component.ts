...
export class AdmAseguradoraFormComponent {
  ...
  flujoCortoCheckBox(event: any) {
    if (!event.target.checked) {
      return;
    }
    this.#swal
      .question({
        text: '¿Realmente requiere activar el flujo corto?',
      })
      .then((response) => {
        if (response.isConfirmed) {
          this.formAseguradora.controls['flujoCorto'].setValue(true);
        } else {
          this.formAseguradora.controls['flujoCorto'].setValue(false);
        }
