...
import { IDocumento, IDocumentoGetCatalogs, IDocumentoEdit } from '../../../helpers/interfaces/adm-documento';
...
export class AdmDocumentoFormComponent {
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(params => {
      ...
      this.documentoService.getCatalogs().subscribe((catalogos: IResponse<IDocumentoGetCatalogs>) => {
      ...
      if (this.documentoId) {
        ...
        this.documentoService.getByIdToEdit(this.documentoId)
          .subscribe({
            next: (response: IResponse<IDocumentoEdit>) => {
              if (response.success) {
                ...
                setTimeout(() => { this.formDocumentos.patchValue(documento); this.changeCategoria(); }, 100);
      ...
      this.formDocumentos.controls['pais'].valueChanges.subscribe((clave) =>
  ...
  /**
   * SINIESTROS Y EMISION
   * Al seleccionar la categoria de sistema se permite escribir la clave del documento
   * Esta clave es un identificador unico 
   */
  changeCategoria() {
    const categoria = this.formDocumentos.controls['categoria'].value;
    $('#div_nombre').removeClass();
    if (categoria == 4)
      $('#div_nombre').addClass('col-md-8')
    else
      $('#div_nombre').addClass('col-md-12')
  }
