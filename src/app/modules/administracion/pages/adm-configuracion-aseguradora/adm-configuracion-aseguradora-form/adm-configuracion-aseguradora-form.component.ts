...
export class AdmConfiguracionAseguradoraFormComponent {
  ...
  selectAseguradora(){
    this.frmConfiguracionAseguradora.get('aseguradora')?.valueChanges.pipe(
      debounceTime(600),
      switchMap(value => this.configuracionAseguradoraService.getCatalogs(value || null)))
      .subscribe({
  ...
  initForm() {
    ...
    this.frmConfiguracionAseguradora.controls[
      'proyecto'
    ].valueChanges.subscribe((idProyecto) => {
      if (idProyecto) {
        this.proyectoFilter = [
          ...new Set(
            this.proyectoAux.filter((element) => element._id === idProyecto)
          ),
        ];
  ...
  onChangeAseguradora(aseguradora: any, event: any) {
    if (event.isUserInput) {
      ...
      if (
          ...
          next: (response: IResponse<IConfiguracionAseguradoraCatalogs>) => {
            if (response.success) {
              ...
              this.oficinasCatalogoFilter = [
                ...new Set(
                  this.oficinasCatalogo.filter((element) =>
                    aseguradora.oficinas.some(
                      (item: any) => item === element.clave
                    )
                  )
                ),
              ];
        ...
        if (this.idProceso != KYCProceso.FIANZAS) this.setInicializarOficina();
      }
    }
  }
  ...
  checkActionAndLoad(_id: string | null): void {
    if (_id) {
      if (this.action === EFormAction.VIEW) this.disableInput();
        ...
        .subscribe({
          next: (response: IResponse<IConfiguracionAseguradoraEdit>) => {
            if (response.success) {
              ...
              const getNameAseguradora = this.aseguradoraAux.find((p) => p._id == configuracionAseguradora.aseguradora);
              configuracionAseguradora.aseguradora = getNameAseguradora?.nombreComercial??'';
              ...
              const oficinas = this.getOficinas;
              oficinas.clear();
              configuracionAseguradora.oficinas.forEach((x, i) => {
                this.setOficina(x, i);
              });

              if (configuracionAseguradora.oficinas.length > 1)
                this.flagOficinas = true;
  ...
  submit(): void {
    ...
    if (this.frmConfiguracionAseguradora.valid) {
      const configuracionAseguradora: IConfiguracionAseguradora = <
        IConfiguracionAseguradora
      >this.frmConfiguracionAseguradora.value;

      if (
        this.idProceso == KYCProceso.FIANZAS &&
        configuracionAseguradora.oficinas.length == 0
      )
        return;

      if (
        configuracionAseguradora.oficinas.filter((x) => x.correos.length == 0)
          .length > 0
      ) {
        this.notifierService.warning('Escriba al menos un correo electrónico');
        return;
      }

      const asegu = this.aseguradora.filter((element) => element.nombreComercial === configuracionAseguradora.aseguradora);
      configuracionAseguradora.aseguradora = asegu[0]._id;
  ...
  setOficina(oficina: any, index: number) {
    ...
    oficina.correos.forEach((correo: any) => {
      correos.value.push(correo);
    });
  }
  ...
  add(event: MatChipInputEvent, oficina: string, index: number): void {
    ...
    if (!this.PATTERN_EMAIL.test(value)) {
      this.notifierService.warning('Escriba un correo electrónico válido');
      event.chipInput.clear();
      return;
    }

    if (correos.value.filter((f: any) => f == value).length > 0) {
      this.notifierService.warning('El correo electrónico ya esta registrado');
      event.chipInput.clear();
      return;
    }
