...
export class AdmProyectoFormComponent implements OnInit {
        ...
        next: (response: IResponse<IProyectoEdit>) => {
          if (response.success) {
            ...
            if (proyecto.proceso == EProceso.KYC)
              this.ramo = [
                ...new Set(
                  this.ramoAux.filter((r) =>
                    [
                      ERamo.CUATRO_NUEVE_DOS,
                      ERamo.FINANCIERAS,
                      ERamo.NO_APLICA,
                    ].includes(r.clave)
                  )
              ...
              this.ramo = this.ramoAux.filter(
                (r) =>
                  ![ERamo.CUATRO_NUEVE_DOS, ERamo.FINANCIERAS].includes(r.clave)
              );
  ...
  onChangeProceso(proceso: EProceso): void {
    this.ramo = this.ramoAux.filter((r) =>
      [ERamo.CUATRO_NUEVE_DOS, ERamo.FINANCIERAS, ERamo.NO_APLICA].includes(
        r.clave
      )
    );
