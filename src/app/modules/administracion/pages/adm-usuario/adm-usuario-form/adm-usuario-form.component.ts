...
export class AdmUsuarioFormComponent implements OnInit, AfterViewInit {
  ...
  initForm(): void {
    this.frmUsuario = this.formBuilder.group({
      nombre: [
        ...
        [
          Validators.required,
          Validators.pattern(AppConsts.SETTINGS.PATTERNS.OnlyLettersWithSpaces),
        ],
      ],
      primerApellido: [
        '',
  ...
  onCheckToAddProject(): boolean {
    const idProyecto = this.frmUsuario.get('proyecto')?.value;
    if (idProyecto) {
      const project = this.proyectos.find((x) => x._id == idProyecto);
      if (
        project?.codigo.includes('EMISIÓN') ||
        project?.codigo.includes('SINIESTROS')
      ) {
